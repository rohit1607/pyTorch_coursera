...
# Contnuous states. Continuous Actions
class ContGridWorld_v2(gym.Env):
    
    def __init__(self, state_dim=2, action_dim=1, grid_dim=[10.,10.],start_pos=[5.0,2.0], target_pos=[8.0,8.0], target_rad=1, F=1):
        super(ContGridWorld_v2, self).__init__()
        return 
    ...
    def setup(self, cfg):
        ...
        self.action_space = gym.spaces.Box(low=0, high=2*np.pi, shape=(self.action_dim,))
        ...
        self.start_pos = np.array(self.params["start_pos"], dtype=np.float32).reshape(2,)
